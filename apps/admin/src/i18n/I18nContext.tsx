  ...
  useEffect(() => {
    const handleLanguageChange = (detail: { language: Language }) => {
      if (process.env.NODE_ENV === 'development') {
        console.log('[I18n] Admin MFE received languageChanged event:', detail);
      }
      if (detail?.language) {
    ...
    if (process.env.NODE_ENV === 'development') {
      console.log('[I18n] Language change listener registered for admin MFE');
    }
    
    return () => {
      unsubscribe();
      if (process.env.NODE_ENV === 'development') {
        console.log('[I18n] Language change listener removed for admin MFE');
      }
    };
  }, [i18n, storageService, eventBus]);
